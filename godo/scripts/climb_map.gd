...
var stagnation_threshold: float = 20.0  # Time in seconds before penalty kicks in
    ...
    if timer < 0:
        ...
        for i in range(climbers.size()):
            ...
            if distance_moved < position_tolerance:
                # Climber is stagnant, increase timer
                climber.stagnation_timer += delta
            else:
                # Climber has moved, reset timer and update position
                climber.stagnation_timer = 0.0
                climber_positions[i] = current_pos
            
            
            # Apply stagnation penalty if climber has been still too long
            if climber.stagnation_timer > stagnation_threshold:
...
func spawn_box(distance: float):
    ...
    var random_scale = clamp(abs(randfn(1.3, 0.4)), 0.5, 2.4)  # Normal distribution with mean 1.3 and std dev 0.4
